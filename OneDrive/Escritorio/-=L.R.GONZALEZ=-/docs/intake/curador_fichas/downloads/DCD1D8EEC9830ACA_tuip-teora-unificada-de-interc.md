# Ficha Curador SETO - # TUIP‑Σ Teoría Unificada de Interc.txt

| campo | valor |
|---|---|
| Ruta original | `C:\Users\L-Tyr\Downloads\New folder\# TUIP‑Σ Teoría Unificada de Interc.txt` |
| SHA256 | `DCD1D8EEC9830ACA1565C6EFD1C643B74FDA61EAA7C1135EA6A2DEBCD833D77E` |
| Bytes | `59562` |
| Tipo | `file` |
| Estado PSI | `CERTEZA` |
| Status | `REGISTRADO` |
| Clasificacion | `OBSERVACIONISMO_RESEARCH_SYNTHESIS` |
| Lane | `research-boundary` |
| Decision | `RESEARCH_ONLY_WITH_CLAIM_BOUNDARY` |
| ActionGate | `REVIEW` |
| Canonico | `C:\Users\L-Tyr\Downloads\New folder\# TUIP‑Σ Teoría Unificada de Interc.txt` |

## Resumen

Observacionismo/TUIP/PSI synthesis; preserve evidence, inference and unknown labels.

## Sinapsis

- Destino: `docs/intake and research/ after claim review`.
- Evidencia: SHA256 `DCD1D8EEC9830ACA1565C6EFD1C643B74FDA61EAA7C1135EA6A2DEBCD833D77E`.
- Uso permitido: local, curado, sin publicacion externa directa.

## Falsadores

- secret/private marker, hash mismatch, unique content loss, strong claim without validation.
- Si aparece secreto, ruta privada o claim fuerte no validado, el estado cambia a `BLOQUEADO`.
