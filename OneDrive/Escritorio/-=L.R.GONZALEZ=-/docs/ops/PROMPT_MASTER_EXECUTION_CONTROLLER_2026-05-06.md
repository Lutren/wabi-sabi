...
| 7 | Claudio Mission Control / COMMS | `hormiguero-mission-control` | `REVIEW` | activo seguro | consolidar vista local con `docs/ops` + COMMS |
...
| 15 | Matrix / Biblioteca / Mission Control | `hormiguero-mission-control` | `REVIEW` | local-safe | vista local de biblioteca con hashes/gates |
| 16 | AI Browser | `claudio-local-agent` + `curador-seto` | `REVIEW_NO_WEB_ACTION` | specs existentes concurrentes | no navegar; inventario y threat model locales |
...
Continuar con `Prompt 7 - Claudio Mission Control / COMMS` en modo reporte/vista local. No arrancar servidor permanente.
